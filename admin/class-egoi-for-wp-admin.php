...
class Egoi_For_Wp_Admin {
	...
	public function __construct($plugin_name, $version, $debug = false) {
		...
		add_action('woocommerce_order_status_on-hold', array($this, 'hookOrderCheck'), 10, 1);
		add_action('woocommerce_after_checkout_billing_form', array($this, 'hookWooPosForm'), 10);
	...
	public function enqueue_scripts() {
		...
		wp_register_script('custom-script1', plugin_dir_url( __FILE__ ) . 'js/egoi-for-wp-admin_bar.js', array('jquery'));
	...
	public function add_plugin_admin_menu() {
		...
		if($apikey['api_key']){
			...
			add_submenu_page($this->plugin_name, __('Lists', 'egoi-for-wp'), __('Lists', 'egoi-for-wp'), $capability, 'egoi-4-wp-lists', array($this, 'display_plugin_lists_page'));
			add_submenu_page($this->plugin_name, __('Subscribers', 'egoi-for-wp'), __('Subscribers', 'egoi-for-wp'), $capability, 'egoi-4-wp-subscribers', array($this, 'display_plugin_subscriber_page'));
			add_submenu_page($this->plugin_name, __('Subscriber Bar', 'egoi-for-wp'), __('Subscriber Bar', 'egoi-for-wp'), $capability, 'egoi-4-wp-bar', array($this, 'display_plugin_subscriber_bar_page'));
			add_submenu_page($this->plugin_name, __('Forms', 'egoi-for-wp'), __('Forms', 'egoi-for-wp'), $capability, 'egoi-4-wp-form', array($this, 'display_plugin_subscriber_form'));
			add_submenu_page($this->plugin_name, __('Widgets', 'egoi-for-wp'), __('Widgets', 'egoi-for-wp'), $capability, 'egoi-4-wp-widget', array($this, 'display_plugin_subscriber_widget'));
	...
	*/
	public function users_queue(){
		...
		if(isset($_POST['submit']) && ($_POST['submit'])){
		    ...
		    try {
			    ...
			    $result = $api->addSubscriberBulk($listID, $tags, $subs);

		    } catch(Exception $e) {
		    	$this->sendError('WooCommerce ERROR', $e->getMessage());
		    }
	...
	public function hookCartUpdate($cart_id_hash, $product_id, $quantity){
		...
		try {
			...
			if($track){
				...
				$this->trackEngage($data, $products, 1);
	...
	public function hookWooPosForm(){
	...
	public function trackEngage($data = array(), $products, $option = false) {
				...
				if($egoi_int['enable_cf']) {
					...
					if(in_array($form_id, $opt['contact_form'])) {
						...
						if(strpos($result->form, $key_name) !== false){
							...
							$get_name = explode('[text ', $result->form);
							$str = explode(' ', strstr($get_name[1], ']', true));
						...
						$numbers = array('tel' => $tel, 'cell' => $cell, 'bd' => $bd);
						
						$subject = $_POST['your-subject'];
						$error_sent = $result->prop('messages')['mail_sent_ng'];
						$success = $result->prop('messages')['mail_sent_ok'];
						...
						if($get->subscriber->STATUS != '2'){
							...
							if($get->subscriber->EMAIL == $email){
								...
								if($update){
									...
									if($subject){
										$get_tags = $api->getTags($subject);
										if($get_tags['NAME'] != $subject){
							                $add_tag = $api->addTag($subject);
							                $tag = $add_tag->ID;
							            }else{
							                $tag = '';
							                foreach ($get_tags as $tags){
							                    if($tags['NAME'] == $subject){
							                        $tag = $tags['ID'];
							                    }
							                }
							            }
							        }
									$add = $api->editSubscriber($egoi_int['list_cf'], $email, array($tag), $name, $extra_fields, $option, $numbers);
								}
								...
								$add = $api->addSubscriberTags($egoi_int['list_cf'], $email, $tag, $name, 1, $extra_fields, $option, $numbers);
		...
		if($egoi_int['enable_pc']) {
			...
			$email = $data['comment_author_email'];
		 	...
		 	if($check == 'on') {
				...
				$api = new Egoi_For_Wp();	
				$add = $api->addSubscriberTags($egoi_int['list_cp'], $email, $tag, $name, 1);
				...
				if($wpdb->insert_id){
					if(!get_option('egoi_mapping')){
                 	...
                 	foreach ($rows as $post){
						...
						<td><button type='button' id='field_$post->id' class='egoi_fields button button-secondary' data-target='$post->id'>".__('Delete', 'egoi-for-wp')."</button></td></tr>";
                 	}
				}
